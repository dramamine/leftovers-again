...
var movesJson = require('../node_modules/leftovers-again/lib/data/moves.json');
var formatsJson = require('../node_modules/leftovers-again/lib/data/formats.json');

var AndersonBOT001 = function () {
    function AndersonBOT001() {
        _classCallCheck(this, AndersonBOT001);
    }

    _createClass(AndersonBOT001, [{
        key: 'decide',
        ...
        value: function decide(state) {
            ...
            if (state.forceSwitch) {
                //att informations
                var buffer = _damage2.default.calculateStats(state.opponent.active);
                ...
                if (advActivePoke != buffer) {
                    ...
                    myPokes = state.self.reserve.filter(function (mon1) {
                        return !mon1.dead;
                    }).filter(function (mon2) {
                        return !mon2.active;
                    });
                    advAtkSeenMoves = [];
                    advAtkTypes = [];
                    percentBasedOnAdvAtkTypes = [];
                    advActivePoke = _damage2.default.calculateStats(advActivePoke);
                }
                ...
                //se só tenho 1 poke...
                var test = state.self.reserve.filter(function (mon1) {
                    return !mon1.dead;
                }).filter(function (mon2) {
                    return !mon2.active;
                });
                if (test.length === 1) {
                    return new _decisions.SWITCH(test[0].id);
                }
            ...
            if (id[0] === 0) {
                ...
                var _myMon = id[1].id;
                alreadySwitched = true;
                return new _decisions.SWITCH(_myMon);
            }
        }
    }, {
        key: '_pickOne',
        ...
        key: 'checkSuperEffectiveAdvAtk',
        value: function checkSuperEffectiveAdvAtk() {
            ...
            if (numMovesSeen != 0) {
                ...
                for (var i = 0; i < advMoves.length; i++) {
                    ...
                    if (categoryMove === "Physical" || categoryMove === "Special") {
                        ...
                        dmgRate[i] = _typechart2.default.compare(typeMove, myActivePoke.types);
        ...
        key: 'checkFastestPoke',
        value: function checkFastestPoke() {
        ...
        key: 'checkSuperEffective',
        value: function checkSuperEffective(activePoke) {
                ...
                for (var j = 0; j < moves.length; j++) {
                    ...
                    if (moves[j].category === "Physical" || moves[j].category === "Special") {
                        if (moves[j].disabled != true) {
                            dmgRate[j] = _typechart2.default.compare(moves[j].type, advActivePoke.types);
                        }
                    } else {
        ...
        key: 'MaybeShouldISwitch',
        value: function MaybeShouldISwitch() {
            console.log("Maybe i should switch");
            for (var i = 0; i < percentBasedOnAdvAtkTypes.length; i++) {

                if (percentBasedOnAdvAtkTypes[i] >= 45) {

                    var value = _typechart2.default.compare(advAtkTypes[i], myActivePoke.types);
                    if (value >= 2) {
                        return true;
                    }
                }
        ...
        value: function BestSwitchPoke() {
                ...
                if (percentBasedOnAdvAtkTypes[i] >= 45) {
                    ...
                    for (var j = 0; j < myNewPokes.length; j++) {
                        if (myNewPokes[j] != undefined) {
                            var value = _typechart2.default.compare(advAtkTypes[i], myNewPokes[j].types);
                            if (value >= 2) {
            ...
            //select best poke
                    ...
                    //independente de ele todos terem desvantagem agora
        ...
        value: function selectBestAtk() {
            ...
            if (myActivePoke != undefined) {
                ...
                for (var j = 0; j < moves.length; j++) {
                    if (moves[j].category === "Physical" || moves[j].category === "Special") {
                        if (moves[j].disabled != true) {
                            var a = _damage2.default.getDamageResult(myActivePoke, advActivePoke, moves[j]);
                            if (a[1] > atk[1]) {
                                atk[0] = a[0];
                                atk[1] = a[1];
                                toReturn[1] = j;
        ...
        key: 'selectBestStatusMove',
        value: function selectBestStatusMove() {
            var moves = [];
            var toReturn = [0, 0];
            var atk = [0, 0];
            var statusMoves = [];
        ...
        key: 'CheckProbabilities',
        value: function CheckProbabilities() {
                ...
                //aqui entra a estratégia
                        ...
                        if (this.checkFastestPoke()) {
                            ...
                            if (myPokes.length > 1 && alreadySwitched === false) {
                                ...
                                //se nao rola trocar de poke eu vou descter o cacete mesmo
                                console.log("Não preciso trocar, vou mete-le o pau!");
                                //vou usar status negativos ou me bufar
                                buffer2 = this.selectBestStatusMove();
                                if (buffer2 === false) {
    ...
    return AndersonBOT001;
}();

exports.default = AndersonBOT001;