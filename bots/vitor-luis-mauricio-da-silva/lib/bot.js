...
class Mallis {
    decide(state) {
        ...
        if (this._calculateSwitchValue(state, jsonFormats, jsonMoves) >= 75) {
            ...
            if (!mon.dead && !mon.disabled && !mon.active) {
                return new _decisions.SWITCH(mon);
            }
        }
    ...
    _checkOpponentProtected(state, move) {
        if (state.opponent.active.prevMoves[0] == "protect" || state.opponent.active.prevMoves[0] == "detect" || state.opponent.active.prevMoves[0] == "spikyshield" || move.category == "Status" && state.opponent.active.prevMoves[0] == "kingsshield") {
            return true;
        }
        return false;
    }
    ...
    _checkOpponentMagicCoated(state) {
        if (this._checkOpponentAbilities(state, "Magic Bounce") || state.opponent.active.prevMoves[0] == "magiccoat") {
            return true;
        }
        return false;
    }
        ...
        for (var i = 0; i < state.self.reserve.length; i++) {
            ...
            if (moni.dead == true || moni.active == true || moni.disabled == true) {
                ...
                for (var j = 0; j < state.opponent.active.types.length; j++) {
                    switch (_typechart2.default.compare(state.opponent.active.types[j], moni.types)) {
                        case 0:
                ...
                for (var j = 0; j < state.opponent.active.seenMoves.length; j++) {
                    ...
                    switch (_typechart2.default.compare(type, moni.types)) {
                        case 0:
                            this._addValue(i, -10, values);
                            break;
                        case 0.5:
    ...
    _selectMove(state, values) {
        ...
        for (var i = 0; i < state.self.active.moves.length; i++) {
            ...
            if (movei.flags.nonsky && this._checkOpponentAbilities(state, "Levitate") || movei.flags.bullet && this._checkOpponentAbilities(state, "Bulletproof") || movei.flags.powder && (this._checkOpponentType(state, "Grass") || this._checkOpponentAbilities(state, "Overcoat")) || movei.flags.protect && this._checkOpponentProtected(state, movei) || movei.flags.reflectable && this._checkOpponentMagicCoated(state) || movei.flags.snatch && this._checkOpponentSnatching(state) || movei.flags.sound && this._checkOpponentAbilities(state, "Soundproof")) {
                moveValue = -1000;
            } else {
                if (movei.basePower > 0) {
                    ...
                    switch (_typechart2.default.compare(movei.type, state.opponent.active.types)) {
                        case 0:
                            moveValue -= 500;
                            break;
                        case 0.5:
                    ...
                    if (movei.self == true && movei.flags.heal == true) {
                        ...
                        if (movei.category == "Status" && movei.target == 'normal') {
                            if (!state.opponent.active.condition.includes(movei.status) || !(movei.id == 'yawn' && state.opponent.active.condition.includes('slp')) || !(movei.id == 'attract' && (state.opponent.active.gender == state.self.active.gender || typeof state.opponent.active.gender == "undefined")) || !(movei.id == 'stealthrock' && state.opponent.side['stealthrock'] != null)) {
    ...
    _calculateSwitchValue(state, formats, moves) {
        ...
        if (state.forceSwitch || state.teamPreview) {
            ...
            for (var i = 0; i < opponentPossibleMoves.length; i++) {
                ...
                switch (_typechart2.default.compare(moves[moveID].type, state.self.active.types)) {
                    case 0:
                        switchValue -= 20;
                        break;
                    case 0.5:
