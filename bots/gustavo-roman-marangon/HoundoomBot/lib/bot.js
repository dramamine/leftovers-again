...
var _log = require('leftovers-again/lib/log');
...
var _kochance = require('leftovers-again/lib/game/kochance');
...
class HOUNDOOM_BOT {
    ...
    decide(state) {
        ...
        if (action.action == "move") {
            return new _decisions.MOVE(action.object);
        } else {
            return new _decisions.SWITCH(action.object);
        }
    }
    ...
    _decideBestAction(state) {
        ...
        var trapped = false;
        if (state.opponent.active.ability == "Shadow Tag" || state.opponent.active.ability == "Arena Trap") trapped = true;else if (state.opponent.active.abilities[0] == "Shadow Tag" || state.opponent.active.abilities[0] == "Arena Trap") trapped = true;else if (state.opponent.active.abilities[1] == "Shadow Tag" || state.opponent.active.abilities[1] == "Arena Trap") trapped = true;else if (state.opponent.active.abilities["H"] == "Shadow Tag" || state.opponent.active.abilities["H"] == "Arena Trap") trapped = true;
        ...
        for (var i = 0; i < myPokemon.moves.length; i++) {
            if (myPokemon.moves[i] != undefined) {
                if (!myPokemon.moves[i].disabled) {
                    var currentDamageMultiplierResult = _damage2.default.getDamageResult(myPokemon, opponentPokemon, myPokemon.moves[i]);
                    var damageValue = 0;
                    var KOobject;
                    var turnsToKO = -1;
                    var chanceToKO = 0;
                    ...
                    KOobject = _kochance2.default.predictKO(currentDamageMultiplierResult, opponentPokemon);
                    turnsToKO = KOobject.turns;
                    chanceToKO = KOobject.chance;

                    //now we give a score based on the turns it will take to KO the opponent:
                    // 1/turn * chance(100)
                    if (turnsToKO != null) currentScore += 1 / turnsToKO * chanceToKO;else currentScore += 0;
    ...
    _selectBestSeenMove(myPokemon, opponentPokemon) {
        ...
        for (var i = 0; i < myPokemon.seenMoves.length; i++) {
            var currentDamageMultiplierResult = _damage2.default.getDamageResult(myPokemon, opponentPokemon, myPokemon.seenMoves[i]);
            var damageValue = 0;
            var KOobject;
            var turnsToKO = -1;
            var chanceToKO = 0;
    ...
    _selectBestPokemon(myPokemon, opponentPokemon) {
        ...
        for (var i = 0; i < myPokemon.length; i++) {
            ...
            for (var j = 0; j < myPokemon[i].types.length; j++) {
                currentAdvantages *= _typechart2.default.compare(myPokemon[i].types[j], opponentPokemon.types);
            }

            //compute the movepool effectivenes agains the opponent
            for (var j = 0; j < myPokemon[i].moves.length; j++) {
                var currentMoveDamage = _damage2.default.getDamageResult(myPokemon[i], opponentPokemon, myPokemon[i].moves[j]);
                var damageValue = 0;
