...
class MeetTheFakers extends _ai2.default {
  constructor() {
    super();
  }
...
- Close Combat`;
  ...
  chooseForJynx(state) {
    ...
    const isCrying = state.opponent.active.boosts && (state.opponent.active.boosts.spd || 0) < 0;

    const shouldKiss = state.opponent.active.statuses.indexOf('slp') === -1 && state.opponent.active.types.indexOf('Ghost') === -1 && !isCrying;

    const shouldTears = !isCrying;

    const pickRandomly = shouldKiss && shouldTears ? Math.random() <= 0.5 : null;
  ...
  chooseForLudicolo(state) {
    ...
    const isRaining = state.weather.indexOf('rain') >= 0 || state.self.active.prevMoves.indexOf('raindance') >= 0;
    ...
    const waterEffectiveness = _typechart2.default.compare('Water', state.opponent.active.types);
    ...
    if (!shouldSurf && !shouldToxic) {
      ...
      const x = state.self.reserve.filter(mon => !mon.dead).sort((a, b) => {
  ...
  shouldFakeout(state) {
    return state.self.active.prevMoves.length === 0 && state.opponent.active.types.indexOf('Ghost') === -1 && !state.self.active.moves.find(move => move.id === 'fakeout').disabled;
  }
