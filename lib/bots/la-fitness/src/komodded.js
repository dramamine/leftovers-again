...
var _typechart = require('lib/typechart');

var _typechart2 = _interopRequireDefault(_typechart);

var _probabilityDistributions = require('./probability-distributions');

var _probabilityDistributions2 = _interopRequireDefault(_probabilityDistributions);
...
class KOModded {
  static predictKO(damage, defender, field = '', hits = 1, isBadDreams = false) {
    ...
    if (!defender.hp || !defender.maxhp) {
      defender = _damage2.default.assumeStats(defender); // eslint-disable-line
    }
    ...
    if (field.isSR && defender.ability !== 'Magic Guard') {
      const effectiveness = _typechart2.default.compare('Rock', defender.types);
      hazards += Math.floor(effectiveness * defender.maxhp / 8);
    }
    if (defender.types.indexOf('Flying') === -1 && ['Magic Guard', 'Levitate'].indexOf(defender.ability) === -1 && defender.item !== 'Air Balloon') {
      if (field.spikes === 1) {
    ...
    if (field.weather === 'Sun') {
      ...
      if (['Rock', 'Ground', 'Steel'].indexOf(defender.types[0]) === -1 && ['Rock', 'Ground', 'Steel'].indexOf(defender.types[1] || null) === -1 && ['Magic Guard', 'Overcoat', 'Sand Force', 'Sand Rush', 'Sand Veil'].indexOf(defender.ability) === -1 && defender.item !== 'Safety Goggles') {
        eot -= Math.floor(defender.maxhp / 16);
      }
    } else if (field.weather === 'Hail') {
      if (defender.ability === 'Ice Body') {
        eot += Math.floor(defender.maxhp / 16);
      } else if (defender.types.indexOf('Ice') === -1 && ['Magic Guard', 'Overcoat', 'Snow Cloak'].indexOf(defender.ability) === -1 && defender.item !== 'Safety Goggles') {
        eot -= Math.floor(defender.maxhp / 16);
      }
    }
    ...
    if (field.terrain === 'Grassy') {
      if (field.isGravity || defender.types.indexOf('Flying') >= 0 && defender.item !== 'Air Balloon' && defender.ability !== 'Levitate') {
        eot += Math.floor(defender.maxhp / 16);
      }
    }
    let toxicCounter = 0;
  ...
  static _getKOChance(damage, hits, hp, eot = 0, maxHP = 400, toxicCounter = 0, isNatureKnown = false) {
  ...
  static _getDistribution(hits, isNatureKnown) {
    return _probabilityDistributions2.default.find(distro => distro.hits === hits && !!distro.nature === isNatureKnown && !distro.crits);
  }

  static _softCritCalc(dmgPct, hits, mean, variance) {
    const distribution = new _gaussian2.default(mean, variance);
