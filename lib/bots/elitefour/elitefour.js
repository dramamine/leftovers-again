...
class EliteFour extends _ai2.default {
  constructor(meta) {
    super(meta);
  ...
  decide(state) {
    ...
    state.self.active.moves.forEach(move => {
      ...
      if (move.category === 'Status' && move.id !== this.lastMove && move.boosts) {
  ...
  sumFitness(obj) {
    ...
    for (const key in obj) {
      if (weights[key]) {
        ...
        const value = weights[key].value ? weights[key].value(obj[key]) : +obj[key];
