...
class Pokemon {
  ...
  data() {
    ...
    const out = {};
    ['dead', 'condition', 'statuses', 'id', 'species', 'moves', 'level', 'gender', 'hp', 'maxhp', 'hppct', 'active', 'events', 'types', 'baseStats', 'ability', 'abilities', 'baseAbility', 'weightkg', 'nature', 'stats', 'position', 'owner', 'item', 'boosts', 'prevMoves', 'order', 'nickname', 'seenMoves'].forEach(field => {
  ...
  static updateMoveList(moves) {
    return moves.map(move => {
      ...
      const research = _pokeutil2.default.researchMoveById(move);
      const out = {};
      ['accuracy', 'basePower', 'category', 'id', 'name', 'volatileStatus', 'priority', 'flags', 'heal', 'self', 'target', 'type', 'pp', 'maxpp'].forEach(field => {
   ...
   */
  recordMove(move) {
    const id = _pokeutil2.default.toId(move);
