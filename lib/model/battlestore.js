...
var BattleStore = function () {
  ...
  _createClass(BattleStore, [{
    ...
    value: function handleMove(actor, move, target) {
      ...
      var targetMon = this.barn.find(target);
      this.events.push({
        type: 'move',
    ...
    key: 'handleCant',
    value: function handleCant(target, reason) {
    ...
    value: function data() {
      ...
      if (output.self.active.length > 1) {
        var zoroark = output.self.active.find(function (mon) {
          return mon.id === 'zoroark';
        });
        if (zoroark) {
          ...
          output.self.active.map(function (mon) {
            mon.isZoroark = true;
          });
        } else {
