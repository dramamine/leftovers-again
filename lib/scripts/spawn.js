...
getServer().then(() => {
  const folders = _glob2.default.sync('*/', { cwd: __dirname + '/../lib/bots/' });
  // console.log(folders);
  const validOptions = folders.map(txt => {
    return txt.replace('/', '');
  }).filter(txt => {
  ...
  _inquirer2.default.prompt([{
    ...
    // console.log(folder);
    const botfiles = _glob2.default.sync('**/*.js', { cwd: __dirname + '/../lib/bots/' + folder.format });
    // console.log(botfiles);
    const choices = botfiles.map(txt => {
