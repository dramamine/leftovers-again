...
class Team {
  ...
  static random(seed = undefined) {
    const data = _fs2.default.readFileSync(__dirname + '/data/randomteams.txt', 'utf8');
   ...
   */
  static _getNextSeed() {
    const data = _fs2.default.readFileSync('./tmp/lastseed', 'utf8');
  ...
  static packTeam(team) {
    ...
    for (let i = 0; i < team.length; i++) {
      ...
      const id = _pokeutil2.default.toId(set.species);
      ...
      // ability
      const abilities = set.abilities ? set.abilities : _pokeutil2.default.researchPokemonById(id).abilities;

      // @TODO
      // suspicious of this. what if we just sent 'id' instead of these 0,1,H shortcuts?
      const ability = _pokeutil2.default.toId(set.ability);
      if (abilities) {
